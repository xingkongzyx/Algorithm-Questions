...
! 注意 240 便无法使用这个二分查找 这道题有一个条件是 The first integer of each row is greater than the last integer of the previous row. 相当于数组变为一维的话就是有序数组. 而 lc240  中 假设 matrix = [[1,4,7], [2,5,8], [3,6,9]], target = 5 便无法用这个方法解决，在这道题(74)当我们获得 targetRow 的时候，能够确定他前面一行的的数字都比 m[targetRow][0] 小，而这个行首又是最后一个小于等于 target 的行首，如果target 存在的话，一定存在于这一行。但是 对于 lc240 来说，targetRow 的上一行也有元素大于行首，可能存在等于target的值
...
class Solution:
    ...
    def searchRow(self, matrix, target):      
        rows = len(matrix)
        ## 先找 target 的所在行, 或者说找到最后一个满足 matrix[x][0] <= target 的行号
        left = 0
        right = rows - 1
    ...
    def searchCol(self, matrix, target, targetRow):
        cols = len(matrix[0])
        ## 再在 targetRow 这一行找 target 的所在列, 或者说找到最后一个满足 matrix[targetRow][x] <= target 的列号
        left = 0
        right = cols - 1 
        while left < right:
            mid = left + (right - left + 1) // 2
...
res = Solution().searchMatrix(matrix = [[1,4,7], [2,5,8], [3,6,9]], target = 5)
print(res)
