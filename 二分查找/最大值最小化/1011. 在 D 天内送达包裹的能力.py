...
class Solution:
    def shipWithinDays(self, weights, days):
        ...
        while left < right:
            ...
            print(
                f"left is {left}, mid is {mid}, right is {right}, time is {neededDays}")
            if neededDays > days:
    ...
    def calculateShipDays(self, weightCapacity, weights):
        ...
        for itemWeight in weights:
            # * 如果当天再继续运输当前item, 当天运输的总的重量就会超过传入函数的 ｢船的运载能力｣, 是不被允许的。所以当前的 item 只能下一天在运输, 运输天数也增加了一天
