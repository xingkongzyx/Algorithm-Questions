...
! h 指数是论文数量，不是引用次数。
* 题目希望我们找一条分割线(也就是下面代码中的mid), 这条分割线的分割点是从 0 到 "len(citations)- 1", 分割线右边的论文数量是 len(citations) - mid, 这样定义分割线的右边界能保证分割线右边最少有一篇论文. 
...
# 这个问题要我们找的下标 i 满足：citations[mid] >= len - mid ，对这个条件取反，那就是 mid 在满足 citations[mid] < len - mid 的时候不是解，说明 mid 太小了，应该在 [mid + 1, right] 里继续查找。可以比较容易找到继续搜索的方向。
* 找到分割线后，分割线右边的论文数量即为所求 h-index. 公式是(totalCitations - mid) 
"""
