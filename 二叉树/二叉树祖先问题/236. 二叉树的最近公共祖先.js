...
* 本题比较关键的地方是递归函数不能直接理解成『找 p 和 q 节点的最近公共祖先』. 这么设计的话 leftRes 或者 rightRes 就不可能有都不为 null 的情况. 如果这两个的值都不为 null, 意思就是 root 的左子树里面可以找到 『p 和 q 节点的最近公共祖先』, root 的右子树里面也可以找到『p 和 q 节点的最近公共祖先』。既然在递归左右子树后就已经获得了『p 和 q 节点的最近公共祖先』, 为什么还需要后面针对当前节点进行判断呢, 逻辑无法说通, 所以将递归函数的功能理解为〖帮 p 或者 q 找到一个最近的祖先〛就行. 更具体地说函数功能分为三个更加符合逻辑. 

/ lowestCommonAncestor 这个函数不要理解为「找最近公共祖先」, 而就理解为帮两个节点「找最近的祖先」. 传入的值是 root, p, q, 在「以 root 为根节点的子树中」帮 p 和 q 找到一个祖先就行, 找到两个就更好了, 如果找不到祖先就返回 null. 在「以 root->left 为根节点的子树」里面找一次, 在「以 root->right 为根节点的子树」里面再找一次. 如果某一边返回值是 null, 那么说明「p 的祖先」和「q 的祖先」都在另一边. 由于找的时候, 一定是找的「最近的祖先」返回, 所以这里直接返回前面的返回值就行了, 可以保证是最近的公共祖先. 如果左右的返回值都不是null, 那说明「p 的祖先」和「q 的祖先」分别在两边, 则当前节点就是「最近公共祖先」. 左右都找不到就直接返回 null
