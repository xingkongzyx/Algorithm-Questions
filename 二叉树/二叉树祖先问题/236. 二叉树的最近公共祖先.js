...
* 本题比较关键的地方是递归函数不能直接理解成『找 p 和 q 节点的最近公共祖先』. 这么设计的话 leftRes 或者 rightRes 就不可能有都不为 null 的情况. 如果这两个的值都不为 null, 意思就是 root 的左子树里面可以找到 『p 和 q 节点的最近公共祖先』, root 的右子树里面也可以找到『p 和 q 节点的最近公共祖先』。既然在递归左右子树后就已经获得了『p 和 q 节点的最近公共祖先』, 为什么还需要后面针对当前节点进行判断呢, 逻辑无法说通, 所以将递归函数的功能理解为〖帮 p 或者 q 找到一个最近的祖先〛就行
