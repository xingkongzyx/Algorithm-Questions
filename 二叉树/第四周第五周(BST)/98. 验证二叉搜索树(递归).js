 ...
 * 1. 确定递归函数的参数和返回值
 *  递归函数的参数传入的就是根节点和当前限定这个节点的「最大值」和「最小值」, 返回的boolean value
 ...
 * ✔️ 当当前节点不在 [minVal, maxVal] 的区间时, 这个节点不能同时符合二叉搜索树的两个特征, 返回 False
 *
 * 3. 确定单层递归的逻辑
 > 前序遍历, 
 * 首先判断当前节点是否满足 (minVal, maxVal) 的区间.然后根据左/右节点传入对应的maxVal 以及 minVal, 一旦发现某一子树不符合要求, 就返回 false。
