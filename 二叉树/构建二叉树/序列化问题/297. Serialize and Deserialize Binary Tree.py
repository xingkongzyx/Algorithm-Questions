"""
# 重要的注意点是为空树时要用 "X" 代替, 这样才能先序唯一确定一棵树。如果没有 "X" 表示空树, 需要〔先序加中序〕或者〔后序加中序〕才能唯一确定一棵树。强制为空树加上 "X" 表示反而简化了这个问题。因为有 "X", 所以在反序列化的过程中可以通过遇到 "X" 返回 None 使得能够成功区分左右子树。 
...
? 为什么使用中序遍历的做法不可以: https://leetcode.cn/problems/serialize-and-deserialize-binary-tree/solution/ji-chong-bu-tong-bian-li-fang-shi-xia-de-0eap/
