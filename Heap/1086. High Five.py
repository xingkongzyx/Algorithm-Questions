class Solution:
    def highFive(self, items):
        rec = {}
        for studentIdx, score in items:
            if studentIdx not in rec:
                rec[studentIdx] = MinHeap()
            studentHeap = rec[studentIdx]
            if studentHeap.size < 5:
                studentHeap.insert(score)
            else:
                if score > studentHeap.peek():
                    studentHeap.remove()
                    studentHeap.insert(score)
        res = []
        for studentIdx in rec:
            studentHeap = rec[studentIdx]
            sum = 0
            while studentHeap.size > 0:
                sum += studentHeap.remove()

            res.append([studentIdx, int(sum / 5)])
        res.sort()
        return res
