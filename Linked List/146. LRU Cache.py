...
# 为什么要是双向链表, 单链表行不行? 另外, 既然哈希表中已经存了 key, 为什么链表中还要存 key 和 val 呢, 只存 val 不就行了? 
# 针对第一个问题来说，因为我们需要删除节点的操作。删除一个节点不光要得到该节点本身的指针，也需要操作其前驱节点的next指针，而双向链表才能支持直接从当前节点查找前驱节点, 保证操作的时间复杂度 O(1)。
# 针对第二个问题来说，当缓存容量已满，我们不仅仅要删除最前面的一个 Node 节点，还要把 map 中映射到该节点的 key 同时删除，而这个 key 只能由 Node 得到。我们可以通过 self.head 获得第一个节点, 但是如果节点中只存储了 val, 我们无法通过 val 删除哈希表中对应的键值对。只有通过 deletedNode.key 才能够进行哈希表中对应键值对的删除
