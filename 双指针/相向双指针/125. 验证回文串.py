...
#! 这道题最重要的是确保 s[left] 和 s[right] 进行比较的时候指向的都是「有效的字符」且两个指针没有越界. 保证两个都是「有效的字符」需要〚if〛判断句中的〚continue〛, 否则的话遇到 "a  bbA" 中间有两个「非有效的字符」的情况, 将会直接将 " " 与 "b" 进行比较, 得到错误的结果. 通过〚continue〛确保两个指针在比较时都能指向「有效的字符」.
