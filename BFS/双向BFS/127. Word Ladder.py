...
? 必看讲解: https://leetcode.com/problems/word-ladder/discuss/1764371/A-very-highly-detailed-EXPLANATION 
"""
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        while len(beginSet) > 0 and len(endSet) > 0:
            ...
            for currentWord in beginSet:
                ...
                * 在第一次也是唯一一次的BFS遍历中, 从 beginSet = ["hot"] 开始, 此时的 "visited = ["hot", "lot"], currentWord = "hot", 注释中的判断便无法通过, 之后无论进行怎样的变化, 由于 visited 已经包含了这两个单词, 无法满足下面的 "if replacedWord not in visited" 的条件, 最终导致这一次循环后两个队列都为空, 循环终止. 
                if currentWord in endSet:
                            return count + 1
                """
                ...
                for charIdx in range(len(currentWord)):
                    for num in range(26):
                        ...
                        if replacedWord in wordSet:
                            # > 如果在这里进行判断结果就不一样了, 此时 currentWord("hot") 经过变换得到 replacedWord("lot") 是在 endSet 中, 能够被成功捕捉到, 代表出现「交集」, 从 beginWord -> endWord 转化成功, 返回转化所需的步骤 steps.
