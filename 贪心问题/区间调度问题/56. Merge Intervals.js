...
* ② 如果当前遍历到的区间的左端点「<= prevIntervalEnd」, 说明它们有交集, 此时产生合并操作, 即: 对 prevIntervalEnd 更新（取两个区间的最大值）. 

# 注意: 我们是「先合并」, 遇到不重合的区间「再推入」prevInterval. 当考察完最后一个区间, 后面没区间了, 遇不到不重合区间, 最后的 prevInterval 没推入 res, 要记住单独补上. 
...
var merge = function (intervals) {
    ...
    for (let i = 1; i < intervals.length; i++) {
        ...
        if (curIntervalStart <= prevIntervalEnd) {
            // * 原则上要更新的 prev 区间的两端, 但因为最开始是按区间的左端升序排序, 能够保证 prev[0] < cur[0]
