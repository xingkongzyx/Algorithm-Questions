 ...
 * 用区间的尾部排序貌似效率会更好, 因为已经保证后面的区间右侧的值都是大于当前区间右侧的值, 所以将发射点设置在右侧边界, 后面的区间只要左边界比它更靠左, 则可以被一起处理掉。
 * 这里换个example: [[10,16], [2,5], [1,6], [7,12]] 为例子:
 *
 * ➀ 先排序, 按区间结束位置排序, 排序后: [[2,5], [1,6], [7,12], [10,16]]
 * ➁ 遍历计算交叉区间,
 *   Ⅰ 发射点初始化为pos = 5, 需要的箭数量 arrows = 1;
 *   Ⅱ 区间[1, 6], 1 是小于5的, 在点 5 射的箭可以干掉这个区间
 *   Ⅲ 区间[7, 12], 在 5 的位置射箭无法打掉, 说明需要增加一个新的发射点, 新的待发射点 pos = 12
 *   Ⅳ 区间[10,16], 10 < 12那么在 12 位置射箭可以干掉它
 *   Ⅴ 返回需要射击点数量
 *
 */
