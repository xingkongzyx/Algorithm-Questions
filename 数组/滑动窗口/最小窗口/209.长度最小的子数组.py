""" 
* 可以使用滑动窗口的原因：如果一个连续子数组的和 ≥ target, 又因为题目中给出的数组是正整数数组, 那么左端点固定, 且长度更长的子串一定满足 和 ≥ target, 但背离「最小长度」的要求. 所以此时选择收缩窗口, 开始考虑固定「右指针」, 向右移动「左指针」的情况了。这是在优化可行解，并让窗口长度挑战最小纪录。符合滑动窗口性质：向右边扩散得到和越来越大, 向左边界扩散得到和越来越小。
...
? https://leetcode.cn/problems/minimum-size-subarray-sum/solution/jin-tian-mei-tu-yi-dong-chuang-kou-xian-zhao-dao-y/
"""
