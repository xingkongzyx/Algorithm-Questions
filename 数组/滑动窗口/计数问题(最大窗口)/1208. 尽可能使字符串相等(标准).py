""" 
#* 可以使用滑动窗口的原因：如果一个连续子串已经不满足其内 ASCII 码差值和「小于等于 cost」, 那么左端点固定, 且长度更长的子串一定也不会是合法区间, 此时的虽然「长度更长」, 但是背离了ASCII 码差值和「小于等于 cost」的要求。 所以此时选择收缩窗口, 开始考虑固定「右指针」, 向右移动「左指针」的情况了。这是在找到满足「ASCII 码差值和小于等于 cost」的可行解, 找到后记录最大的可行子串的长度。符合滑动窗口性质：向右边扩散得到和越来越大, 向左边界扩散得到和越来越小。
