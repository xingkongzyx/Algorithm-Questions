...
❌ 不适用的场景：
1. 求最小窗口的问题
   - 例如：LeetCode 209. 长度最小的子数组
   - 例如：LeetCode 76. 最小覆盖子串
   - 原因：需要找最小值，窗口必须尽可能收缩到最小有效状态
   
2. 需要统计所有满足条件的窗口数量
   - 例如：LeetCode 713. 乘积小于K的子数组（求个数）
   - 例如：LeetCode 992. K个不同整数的子数组（求个数）
   - 原因：需要枚举每个有效窗口，不能跳过较小的窗口
3. 窗口不满足单调性的问题
   - 例如：某些需要窗口内元素满足特定顺序的问题
   - 原因：窗口扩大/缩小时状态不可逆
✅ 适用的场景：
1. 求最大/最长窗口的问题
2. 窗口满足单调性：大窗口无效 → 收缩后可能有效
3. 只关心最终的最大值，不关心具体位置
典型适用题目：
