...
class Solution(object):
    ...
    def quickSort(self, left, right, nums):
        ...
        """
        i = left + 1
        # 当 i 恰好等于 gt 的时候, 循环不需要继续下去. 因为此时整个数组都已经被遍历完毕。i 等于 gt 的时候，因为 gt 是属于闭区间，所以 gt 所在的位置以及 gt 后面的元素已经被遍历完成了。然后 i 所在的位置属于开区间，所以 i-1 这个位置，也就是 gt-1 已经被遍历完成了。综上，整个 left 到 right 的闭区间都已经被遍历完成。
        while i < gt:
            if nums[i] < pivot:
                lt += 1
