/* 
* 1. 确定dp数组以及下标的含义
* dp[i]: 凑成目标正整数为i的排列个数为dp[i]. 排列的话 "(1,2)" 以及 "(2,1)" 算是不同的排列
* 2. 确定递推公式
* dp[i] += dp[i - nums[j]]
* 3. dp数组如何初始化
...
/ 而应有的 {3,1} 却永远不会出现, 因为针对 nums[0] = 1 遍历的时候, dp[4]那个时候得到的只能是 dp[4] += dp[4-1]；而 dp[3] 那时因为只处于对 "nums[0] = 1" 进行遍历, dp[3] 那时对应的排列是 {1,1,1}, dp[4] 只是在它的基础上再加了个 1, 所有结果是 {1,1,1,1}. 而 {3,1} 由于 dp[3] 那时没有考虑由 {3} 组成的情况, {3,1} 也无法出现

! 所以遍历的顺序是: target(背包) 放在外循环, 将nums(物品) 放在内循环, 内循环从前到后遍历。
*/
