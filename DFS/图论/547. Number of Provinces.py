...
在这里, 我们可以将城市之间的相连关系看成是图, 矩阵 isConnected 是图的邻接矩阵, 城市是图的节点, 相连关系是图中相连的边, 而要求的省份即是图中的「连通分量」, 那么问题就变成求图中「连通分量」的数量。
...
/ 空间复杂度: O(n) 其中 nn 是城市的数量。需要使用数组 visited 记录每个城市是否被访问过, 数组长度是 n, 递归调用栈的深度不会超过 n。
