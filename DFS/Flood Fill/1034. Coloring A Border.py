...
* 首先通过 DFS 遍历标记所有属于 "connected component" 的节点为「特殊色 -1」, 代表这个节点「被访问过」并且属于 "connected component". 等四周递归完成后，进行边界判断，以及恢复不是边界的节点的颜色。
* 判断的方法是: 检查「当前节点」的上下左右四个节点是否值为 -1 或者值为「旧颜色」, 是的话说明当前节点属于 "connected component" 但同时又不属于 "border of a connected component", 将其恢复成「旧颜色」. 
