...
* 因为在内循环里, "matrix[newRowIdx][newColIdx] > matrix[rowIdx][colIdx]" 确保了之前已经访问过的元素根本就不会出现在后续的路径里, 所以 visited 就没有存在的意义, 甚至可以直接缓存每一个子调用的计算结果, 因为不管你从哪个位置开始迭代, 因为没有状态(没有 visited), 计算结果变成唯一的了——因此变得可以缓存了。
