        ...
        * 用栈保存遇到「左括号」时前面计算好了的结果和运算符。
        ...
        for char in s:
            if char.isdigit():
                ...
                # * 如果当前是 ( , 那么说明之后是「右边的表达式」(注: 我们将括号中的表达式称为「右边的表达式」, 方便与链接中的题解组合理解), 而「右边的表达式」里的内容需要优先计算, 所以要把当前「左边的表达式」的结果res 以及左右表达式之间的「操作符」 sign 进栈, 更新 res 和 sign 为新的开始; 例如当前表达式为：'123+(...)' 则将 "res=+123", 入栈, 将 "sign=+", 入栈
                stack.append(res)
                stack.append(sign)
