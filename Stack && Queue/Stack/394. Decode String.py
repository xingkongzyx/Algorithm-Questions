...
需要解决两个问题：
    1. 如何解码类似于 "3[c]" 的字符串 
    2. 如果出现嵌套结构, 如何保证解码的正确性
遇到']'就代表可以执行解码操作, 最重要的是得到 [] 中的字符串以及数字, 完成之后把解码的字符串 push 进栈即可. 到最后, 栈中存储的就是解码序列
! 为什么用栈: 由于解码逻辑是先解码内部结构, 再解码外部结构, 而遍历顺序是从左到右（即从外到内）, 所以想到用栈存储字符串
...
! 这道题可以作为 225 calculator 的前置题, 针对 [] 中的括号处理, 其实就相当于计算器中的 () 的处理
    ...
    def decodeString(self, s) -> str:
        ...
        for char in s:
            if "1" <= char <= "9":
                ...
                # * 要把 [ 前面的数字(用于遇到 「]」 时进行 repeat 计算) 和之前的结果放到 stack 中进行保存，然后计算 [] 中内容
